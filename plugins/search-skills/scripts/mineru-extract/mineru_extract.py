...
Submit a URL (including HTML pages) to MinerU, poll until done,
...
Usage:
  python3 mineru_extract.py "<URL>" --model MinerU-HTML --print
...
def _default_workspace() -> pathlib.Path:
    """Return workspace root for output storage."""
...
def create_task(*, api_base: str, token: str, source_url: str, model_version: str,
                is_ocr: bool | None, enable_formula: bool | None,
                enable_table: bool | None, language: str | None,
                page_ranges: str | None, extra_formats: list[str] | None) -> str:
...
def poll_task(*, api_base: str, token: str, task_id: str,
              timeout_sec: int = 600, poll_interval: float = 2.0) -> dict:
...
def main() -> int:
    ...
    ap.add_argument("--model", dest="model_version", default=None,
                    help="pipeline | vlm | MinerU-HTML (required for HTML).")
    ...
    if not token:
        print("Missing MINERU_TOKEN environment variable", file=sys.stderr)
        return 2
    ...
    task_id = create_task(
        api_base=args.api_base, token=token, source_url=args.source,
        model_version=model_version, is_ocr=args.is_ocr,
        enable_formula=args.enable_formula, enable_table=args.enable_table,
        language=args.language, page_ranges=args.page_ranges,
        extra_formats=extra_formats,
    )

    data = poll_task(api_base=args.api_base, token=token, task_id=task_id,
                     timeout_sec=args.timeout, poll_interval=args.poll_interval)
    ...
    summary = {
        "task_id": task_id, "state": data.get("state"),
        "model_version": model_version, "full_zip_url": full_zip_url,
        "out_dir": str(base_out), "zip_path": str(zip_path),
