...
Expose a stable, workflow-friendly interface for parsing documents.
Accept file_sources (comma/newline-separated URLs), use MinerU API,
download result zip + extract main Markdown, return JSON result contract.
...
Usage:
  python3 mineru_parse_documents.py --file-sources "URL1,URL2" --emit-markdown
...
def _default_workspace() -> pathlib.Path:
    ...
    return pathlib.Path.home() / ".cache" / "claude-search-skills"
...
def poll_task(*, api_base: str, token: str, task_id: str,
              timeout_sec: int, poll_interval: float) -> dict:
    endpoint = api_base.rstrip("/") + f"/api/v4/extract/task/{task_id}"
    start = time.time()
    last_state = None
    while True:
        res = _http_json("GET", endpoint,
...
def extract_main_markdown(zip_bytes: bytes, out_dir: pathlib.Path) -> pathlib.Path | None:
    ...
    return sorted(md_files, key=score, reverse=True)[0]


def parse_one_url(*, api_base: str, token: str, source_url: str, enable_ocr: bool,
                  language: str, page_ranges: str | None, model_version: str | None,
                  enable_table: bool | None, enable_formula: bool | None,
                  extra_formats: list[str] | None, timeout_sec: int,
                  poll_interval: float, cache: bool, force: bool) -> dict:
    mv = _pick_model_version(source_url, model_version)
    payload: dict = {"url": source_url, "model_version": mv, "language": language}
    ...
    result = {
        "ok": True, "source": source_url, "task_id": task_id,
        "state": data.get("state"), "model_version": mv,
        "language": language, "enable_ocr": bool(enable_ocr),
        "page_ranges": page_ranges, "full_zip_url": full_zip_url,
        "out_dir": str(out_dir), "zip_path": str(zip_path),
        "markdown_path": str(md_path) if md_path else None,
        "cached": False, "cache_key": key, "fetched_at": int(time.time()),
    }
...
def main() -> int:
    ...
    ap.add_argument("--file-sources", required=True)
    ...
    ap.add_argument("--extra-formats", default=None)
    ...
    if not token:
        print(json.dumps({"ok": False, "error": "Missing MINERU_TOKEN env var", "items": []},
                          ensure_ascii=False))
        return 2
    ...
    for src in sources:
        if not _is_url(src):
            errors.append({"source": src, "error": "Local file paths not supported yet."})
            continue
        try:
            meta = parse_one_url(
                api_base=api_base, token=token, source_url=src,
                enable_ocr=args.enable_ocr, language=args.language,
                page_ranges=args.page_ranges, model_version=args.model_version,
                enable_table=enable_table, enable_formula=enable_formula,
                extra_formats=extra_formats, timeout_sec=args.timeout,
                poll_interval=args.poll_interval, cache=args.cache, force=args.force,
            )
    ...
    out = {"ok": len(errors) == 0, "items": items, "errors": errors}
