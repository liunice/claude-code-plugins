#!/usr/bin/env python3
"""
Multi-source search: Brave + Exa + Tavily + Grok with intent-aware scoring and ranking.

Four search sources — at least one API key must be configured via environment variables.
Sources without keys are silently skipped.
...
def validate_keys(keys: dict) -> bool:
    """Check that at least one search source API key is configured."""
...
def execute_search(query: str, mode: str, keys: dict, num: int,
    ...
    If sources is set, only run those sources (e.g. {'brave', 'grok', 'exa', 'tavily'}).
    """
    all_results = []
    answer_text = None
    source_status = {}
    ...
    all_source_names = ["brave", "exa", "tavily", "grok"]
    ...
    if mode == "fast":
        ...
        if "exa" in keys and _want("exa"):
            ...
            all_results = res
        elif "brave" in keys and _want("brave"):
            chosen = "brave"
...
def main():
    ap = argparse.ArgumentParser(
        description="Multi-source search (Brave+Exa+Tavily+Grok) with intent-aware scoring")
    ...
    ap.add_argument("--source", default=None,
                    help="Comma-separated sources to use (brave,exa,tavily,grok). Default: all available")
    ...
    # Validate: at least one search source must be configured
    if not validate_keys(keys):
        print(json.dumps({
            "error": "No search API keys configured. Set at least one of: "
    ...
    sources_summary = {}
    for name in ["brave", "exa", "tavily", "grok"]:
